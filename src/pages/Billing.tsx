...
import { Check, CreditCard, AlertTriangle, Sparkles, Crown } from 'lucide-react';
      ...
      <div className="flex flex-col gap-5">
        ...
        <section className="rounded-xl border border-border bg-card p-5">
          ...
          <div className="flex items-center gap-3 mb-2">
            <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold ${
              ...
              {status === 'active' && <><Crown className="h-3 w-3" /> Plano Pro</>}
              {status === 'trial' && <><Sparkles className="h-3 w-3" /> Teste Gratuito</>}
              {status === 'expired' && <><AlertTriangle className="h-3 w-3" /> Expirado</>}
            </span>
          </div>

          <p className="text-sm text-muted-foreground">
            {status === 'trial' && daysLeft > 0 && `Seu teste termina em ${daysLeft} dia${daysLeft !== 1 ? 's' : ''}.`}
            {status === 'trial' && daysLeft === 0 && 'Seu teste termina hoje.'}
            {status === 'active' && 'Seu plano está ativo.'}
        ...
        <section className="rounded-xl border border-border bg-card p-5">
          ...
          <Button className="w-full h-11" onClick={() => setUpgradeOpen(true)}>
            {status === 'active' ? 'Alterar plano' : 'Assinar agora'}
          </Button>
        </section>
        ...
        {status === 'active' && (
          <section className="rounded-xl border border-border bg-card p-5">
            <h2 className="text-sm font-semibold text-foreground mb-3">Ações da Conta</h2>
            <div className="flex flex-col sm:flex-row gap-3">
              <Button variant="outline" className="h-10" onClick={() => setUpgradeOpen(true)}>
                Alterar plano
              </Button>
              <Button variant="outline" className="h-10 text-destructive hover:text-destructive border-destructive/30 hover:bg-destructive/5" onClick={() => setCancelOpen(true)}>
                Cancelar plano
              </Button>
            </div>
          </section>
      ...
      <Dialog open={upgradeOpen} onOpenChange={setUpgradeOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            ...
            <Button onClick={() => { setUpgradeOpen(false); toast.info('Integração de pagamento em breve!'); }}>
              Continuar
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <AlertDialog open={cancelOpen} onOpenChange={setCancelOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Você perderá acesso aos recursos Pro ao final do período atual.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={() => { setCancelOpen(false); toast.info('Cancelamento será disponível em breve.'); }}
            >
