    ...
    <div className="flex flex-col gap-4 p-4">
      ...
      {element.type === 'service' && (
        <div className="space-y-3">
          <Separator />
          <div>
            <Label className="text-xs text-muted-foreground">Quantidade de itens ({element.serviceCount || 3})</Label>
            <Slider
              min={1}
              max={10}
              step={1}
              value={[element.serviceCount || 3]}
              onValueChange={([v]) => onUpdate({ serviceCount: v })}
              className="mt-1"
            />
          </div>
          <div>
      ...
      {element.type === 'total-calculation' && (
        <div className="space-y-3">
          <Separator />
          <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Cálculo Automático</span>
