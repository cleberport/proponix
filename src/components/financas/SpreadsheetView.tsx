...
import {
  Plus, Trash2, MoreHorizontal, ArrowLeft, ArrowRight, GripVertical,
...
export default function SpreadsheetView({ table, onUpdate }: Props) {
  ...
  const renderCell = (row: FinanceRow, col: FinanceColumn) => {
    ...
    if (col.type === 'formula') {
      ...
      return <span className="text-sm font-medium tabular-nums">{formatBRL(result)}</span>;
    }
    ...
    return (
      <div
        className="w-full h-full min-h-[28px] px-1 py-0.5 text-sm cursor-text flex items-center tabular-nums"
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-border/50">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs text-muted-foreground">{rows.length} linhas · {columns.length} colunas</span>
        </div>
      </div>
        ...
        <div className="min-w-full inline-block">
          <table className="w-full border-collapse">
            <thead className="sticky top-0 z-10 bg-muted/80 backdrop-blur-sm">
              <tr>
                <th className="w-10 px-2 py-2 text-center text-xs font-medium text-muted-foreground border-b border-r border-border/50">#</th>
                {columns.map((col, ci) => {
                  const TypeIcon = Icon(col.type);
                  return (
                    <th key={col.id} className="px-2 py-2 text-left text-xs font-medium text-muted-foreground border-b border-r border-border/50 min-w-[120px] max-w-[250px]">
                      <div className="flex items-center gap-1.5">
                        ...
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <Button variant="ghost" size="icon" className="h-5 w-5 ml-auto opacity-0 group-hover:opacity-100 hover:opacity-100 focus:opacity-100 shrink-0" style={{ opacity: undefined }}>
                ...
                <th className="w-10 px-1 py-2 border-b border-border/50">
              ...
              <tfoot>
                <tr className="bg-muted/40 border-t border-border/50">
                  ...
                  {columns.map(col => {
                    ...
                    return (
                      <td key={col.id} className="px-2 py-2 text-sm font-semibold tabular-nums border-r border-border/30">
                        {sum !== null ? (
