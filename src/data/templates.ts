...
export const starterTemplates: Template[] = [
  makeTemplate('corporate-budget', 'Orçamento Corporativo', 'Corporativo', 'Template profissional para orçamentos corporativos com layout estruturado', [
    ...
    el({ type: 'table', x: 40, y: 170, width: 515, height: 200, content: '', rows: [
    ...
    el({ type: 'price-field', x: 350, y: 455, width: 205, height: 30, content: 'Imposto:', variable: 'tax', fieldCategory: 'calculated' }),
    ...
    el({ type: 'notes', x: 40, y: 550, width: 515, height: 100, content: 'Observações: Pagamento em até 30 dias.', fieldCategory: 'default' }),
  ...
  makeTemplate('dj-services', 'Serviços de DJ', 'Entretenimento', 'Template para orçamento de serviços de DJ e música', [
    ...
    el({ type: 'notes', x: 40, y: 510, width: 515, height: 90, content: 'Inclui montagem, passagem de som e desmontagem. Taxa de deslocamento pode ser aplicada para locais acima de 80 km.', fieldCategory: 'default' }),
  ], [...commonVars, 'event_name', 'location', 'service_name', 'price', 'tax_rate'], {
  ...
  makeTemplate('agency-quote', 'Orçamento Agência', 'Agência', 'Orçamento profissional de agência com layout moderno', [
    ...
    el({ type: 'notes', x: 40, y: 580, width: 515, height: 80, content: '50% de entrada para iniciar. Saldo na entrega do projeto.', fieldCategory: 'default' }),
  ], [...commonVars, 'event_name', 'service_name', 'price', 'tax_rate'], {
  ...
  makeTemplate('minimal-invoice', 'Fatura Minimalista', 'Geral', 'Template de fatura ultra-limpo com design mínimo', [
    ...
    el({ type: 'table', x: 40, y: 200, width: 515, height: 180, content: '', rows: [
      { cells: ['Descrição', 'Qtd', 'Valor Unit.', 'Total'] },
      { cells: ['{{service_name}}', '1', '{{price}}', '{{price}}'] },
    ]}),
    el({ type: 'divider', x: 350, y: 410, width: 205, height: 1, content: '' }),
    el({ type: 'total-calculation', x: 350, y: 425, width: 205, height: 30, content: 'Total:', variable: 'total', fontWeight: '600', fontSize: 16, fieldCategory: 'calculated' }),
    el({ type: 'notes', x: 40, y: 520, width: 515, height: 60, content: 'Obrigado pela preferência.', alignment: 'center', color: '#64748B', fieldCategory: 'default' }),
  ], [...commonVars, 'service_name', 'price', 'tax_rate'], {
    defaultValues: { service_name: 'Serviços Profissionais', price: '1000', tax_rate: '0' },
    inputFields: ['client_name', 'event_date'],
    calculatedFields: { subtotal: 'price', tax: 'price * tax_rate', total: 'price + tax' },
    settings: { taxRate: 0, showTax: false },
  }),
